...
)
{
	switch( HostErrno )
	{
		...
		case TRY_AGAIN:			return "Host not found (suggest try again)";
		case NO_RECOVERY:		return "Non-recoverable error: FORMERR, REFUSED, NOTIMP";
		case NO_DATA:			return "No Data (valid name, but no record of requested type)";
		default:
...
)
{
	...
	do
	{
		...
		if( Recvd <= 0 )
		{
			if( TotalRecvd > 0 && (Recvd == 0 || SOCK_ERRNO == EWOULDBLOCK) )
				return TotalRecvd;
			if( SOCK_ERRNO == EINTR )
				continue;
