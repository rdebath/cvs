...
/* Like error, but print RCS-style error messages.  This is only
   necessary in order to produce output identical to RCS; if we ever
   slough off that particular coil, so can go this code. */
/* VARARGS */
void
#if defined (HAVE_VPRINTF) && defined (__STDC__)
rcserror (char *rcsfile, const char *message, ...)
#else
rcserror (rcsfile, message, va_alist)
    char *rcsfile;
    const char *message;
    va_dcl
#endif
{
	...
	{
	    entire = malloc (strlen (mess) + strlen (rcsfile) + 10);
	    if (entire == NULL)
	    {
		free (mess);
		status = 1;
