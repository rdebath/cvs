...
cvsremove (int argc, char **argv)
{
	...
	(remove_fileproc, (FILESDONEPROC) NULL,
	 ...
	 local, W_LOCAL, 0, CVS_LOCK_READ, (char *) NULL, 1, (char *) NULL);
	...
	error (0, 0,
		...
		"%d files exist; remove them first"), existing_files);
...
remove_fileproc (void *callerdat, struct file_info *finfo)
{
	...
	fname = xmalloc (strlen (finfo->file)
			 + sizeof (CVSADM) + sizeof (CVSEXT_LOG) + 10);
	(void) sprintf (fname, "%s/%s%s", CVSADM, finfo->file, CVSEXT_LOG);
	if (unlink_file (fname) < 0 && !existence_error (errno))
	 ...
	 */
	error (0, 0, "\
cannot remove file `%s' which has a numeric sticky tag of `%s'", finfo->fullname, vers->tag);
    }
	 ...
	 */
	error (0, 0, "\
cannot remove file `%s' which has a sticky date of `%s'", finfo->fullname, vers->date);
    }
    else
    {
	char *fname;
