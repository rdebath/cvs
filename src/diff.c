...
static Dtype diff_dirproc (void *callerdat, char *dir,
			   char *pos_repos, char *update_dir, List * entries);
...
RCS file: ", 0);
	...
	if (empty_file == DIFF_ADDED)
	{
	    if (use_rev2 == NULL)
		status =
		    diff_exec (DEVNULL, finfo->file, label1, label2, opts,
			       RUN_TTY);
	    else
	    {
		int retcode;
		...
		status =
		    diff_exec (DEVNULL, tmp, label1, label2, opts, RUN_TTY);
	    }
	}
	else
	{
...
/* ARGSUSED */
static int
diff_dirleaveproc (void *callerdat, char *dir, int err, char *update_dir,
		   List * entries)
{
    return (diff_errors);
}
