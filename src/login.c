...
password_entry_parseline (const char *cvsroot_canonical,
			  const const unsigned char warn,
			  const const int linenumber, char *linebuf)
{
    char *password = NULL;
    char *p;
...
static char *
password_entry_operation (password_entry_operation_t operation,
			  cvsroot_t * root, char *newpassword)
{
	...
	line++;
	password =
	    password_entry_parseline (cvsroot_canonical, 1, line, linebuf);
	if (password != NULL)
	    /*
  ...
  process:
	...
	while ((line_length = getline (&linebuf, &linebuf_len, fp)) >= 0)
	{
		...
		if (fprintf (tmp_fp, "%s", linebuf) == EOF)
		{
		     ...
		     */
		    error (0, errno, "fatal error: cannot write %s",
			   tmp_name);
...
login (int argc, char **argv)
{
	 ...
	 */
	if (!tmp)
	    error (1, errno, "login: Failed to read password.");
...
logout (int argc, char **argv)
{
    ...
    password_entry_operation (password_entry_delete, current_parsed_root,
			      NULL);
