...
wrap_unparse_rcs_options (char **line, int first_call_p)
{
    ...
    *line = xmalloc (strlen (wrap_list[i]->wildCard) + strlen ("\t") + strlen (" -k '") + (wrap_list[i]->rcsOption != NULL ? strlen (wrap_list[i]->rcsOption) : 2) + strlen ("'") + 1);	/* leave room for '\0' */
...
wrap_add (char *line, int isTemp)
{
	...
	++line;
    for (temp = line; *line && !isspace ((unsigned char) *line); ++line)
	;
    if (temp == line)
	return;
...
wrap_fromcvs_process_file (const char *fileName)
{
    ...
    args = xmalloc (strlen (e->fromcvsFilter) + strlen (fileName));
    /*
       FIXME: sprintf will blow up if the format string contains items other
       than %s, or contains too many %s's.  We should instead be parsing
