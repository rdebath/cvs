...
cvstag (int argc, char **argv)
{
	...
	for (i = 0; i < argc; i++)
	{
	    ...
	    history_write ('T', (delete_flag ? "D" : (numtag ? numtag :
			   ...
			   symtag, argv[i], "");
	    err +=
		do_module (db, argv[i], TAG,
			   delete_flag ? "Untagging" : "Tagging", rtag_proc,
			   (char *) NULL, 0, local, run_module_prog, 0,
			   symtag);
	}
	close_module (db);
    }
	   ...
	   int shorten, int local_specified, char *mname, char *msg)
{
	...
	repository =
	    xmalloc (strlen (current_parsed_root->directory) +
		     strlen (argv[0]) + (mfile ==
					 NULL ? 0 : strlen (mfile) + 1) + 2);
	(void) sprintf (repository, "%s/%s", current_parsed_root->directory,
			argv[0]);
	where =
	    xmalloc (strlen (argv[0]) +
		     (mfile == NULL ? 0 : strlen (mfile) + 1) + 1);
	...
	(is_rtag ? rtag_fileproc : tag_fileproc,
	 ...
	 local_specified, which, 0, CVS_LOCK_WRITE, where, 1, repository);
    dellist (&mtlist);
    if (which & W_REPOS)
	free (repository);
    if (where != NULL)
...
check_fileproc (void *callerdat, struct file_info *finfo)
{
	...
	p->key = xstrdup (xdir);
	p->type = UPDATE;
	ml = (struct master_lists *) xmalloc (sizeof (struct master_lists));
	ml->tlist = tlist;
	p->data = (char *) ml;
		     ...
		     List * entries)
{
    ...
    ppd.symtag = symtag;
    if ((n =
	 Parse_Info (CVSROOTADM_TAGINFO, repos, pretag_proc, PIOPT_ALL,
		     &ppd)) > 0)
    {
...
rtag_fileproc (void *callerdat, struct file_info *finfo)
{
    ...
    if (numtag
	&& isdigit ((unsigned char) *numtag) && strcmp (numtag, version) != 0)
    {
...
/* ARGSUSED */
static Dtype
tag_dirproc (void *callerdat, char *dir, char *repos, char *update_dir,
	     List * entries)
{
...
static Dtype
val_direntproc (void *callerdat, char *dir, char *repository,
...
Numeric tag %s contains characters other than digits and '.'", name);
	...
	if (db == NULL)
	{
	    ...
	    db = dbm_open (valtags_filename, O_RDWR | O_CREAT | O_TRUNC,
			   0666);
	    (void) umask (omask);
