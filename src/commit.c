...
static Dtype
find_dirent_proc (void *callerdat, char *dir, char *repository,
...
static int
find_filesdoneproc (void *callerdat, int err, char *repository,
		    char *update_dir, List * entries)
{
    struct find_data *find_data = (struct find_data *) callerdat;
...
#  endif
	 ...
	 */
	find_args.argv =
	    (char **) xmalloc (find_args.argc * sizeof (char **));
	find_args.argc = 0;
...
classify_file_internal (struct file_info *finfo, Vers_TS ** vers)
{
	...
	status =
	    Classify_File (finfo, saved_tag, (char *) NULL, (char *) NULL, 1,
			   0, vers, 0);
    noexec = save_noexec;
    quiet = save_quiet;
...
check_fileproc (void *callerdat, struct file_info *finfo)
{
    ...
    if (strncmp
	(finfo->repository, current_parsed_root->directory, cvsroot_len) == 0
	&& ISDIRSEP (finfo->repository[cvsroot_len])
	&& strncmp (finfo->repository + cvsroot_len + 1, CVSROOTADM,
	...
	case T_REMOVED:
		...
		if (file_has_markers (finfo))
		{
		     ...
		     */
		    error (0, 0, "\
warning: file `%s' seems to still contain conflict indicators", finfo->fullname);
		}
	    }

	    if (status == T_REMOVED
		&& vers->tag && isdigit ((unsigned char) *vers->tag))
	    {
		/*
...
precommit_list_proc (Node * p, void *closure)
{
    ...
    if (li->type == T_ADDED
	|| li->type == T_MODIFIED || li->type == T_REMOVED)
    {
	run_arg (p->key);
    }
    return (0);
...
commit_fileproc (void *callerdat, struct file_info *finfo)
{
	...
	if (rev != NULL && !RCS_nodeisbranch (finfo->rcs, write_dirtag))
    ...
    {
	if (checkaddfile
	    (finfo->file, finfo->repository, ci->tag, ci->options,
	     &finfo->rcs) != 0)
	{
	 ...
	 */
	err =
	    finaladd (finfo, ci->rev ? ci->rev : xrev, ci->tag, ci->options);
	if (xrev)
	    free (xrev);
    }
...
/* ARGSUSED */
static int
commit_filesdoneproc (void *callerdat, int err, char *repository,
		      char *update_dir, List * entries)
{
    Node *p;
    List *ulist;
...
remove_file (struct file_info *finfo, char *tag, char *message)
{
    ...
    if (retcode != 0)
    {
	error (0, 0, "failed to check out `%s'", finfo->fullname);
	return (1);
    }
	      ...
	      RCSNode ** rcsnode)
{
	...
	if (adding_on_branch)
	{
	    ...
	    (void) umask (omask);
	    (void) sprintf (rcs,
			    "%s/%s/%s%s", repository, CVSATTIC, file, RCSEXT);
	}
	else
	{
	    rcs = xmalloc (strlen (repository)
			   + strlen (file) + sizeof (RCSEXT) + 2);
	...
	if (add_rcs_file (NULL, rcs, file, NULL, opt,
			  NULL, NULL, 0, NULL, desc, desclen, NULL) != 0)
	{
	    retval = 1;
	    goto out;
	}
