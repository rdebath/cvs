...
start_recursion (FILEPROC fileproc, FILESDONEPROC filesdoneproc,
		 ...
		 void *callerdat, int argc, char **argv, int local, int which,
		 int aflag, int locktype, char *update_preload, int dosrcs,
		 char *repository_in)
    /*
     ...
     */
{
...
#ifdef CLIENT_SUPPORT
	if (!just_subdirs
	    && CVSroot_cmdline == NULL && current_parsed_root->isremote)
	{
	...
	{
	    dirlist =
		Find_Directories ((char *) NULL, W_LOCAL, (List *) NULL);
	    /*
	...
	else
	{
		    ...
		    )
		{
		    ...
		    reposfile = xmalloc (strlen (repos) + strlen (comp) + 5);
		    (void) sprintf (reposfile, "%s/%s", repos, comp);
		    free (repos);

		    if (!wrap_name_has (comp, WRAP_TOCVS)
			&& isdir (reposfile))
			addlist (&dirlist, argv[i]);
		    else
			addfile (&files_by_dir, dir, comp);
...
do_recursion (struct recursion_frame *frame)
{
	...
	if (frame->flags != R_SKIP_DIRS)
	    dirlist =
		Find_Directories (process_this_directory ? repository : NULL,
				  frame->which, entries);
    }
    else
    {
...
do_file_proc (Node * p, void *closure)
{
	 ...
	 */
	if (finfo->rcs == NULL && !(frfile->frame->which & W_LOCAL))
	{
...
do_dir_proc (Node * p, void *closure)
{
	...
	if (!printed_cvs_msg)
	{
	    error (0, 0, "warning: directory %s specified in argument", dir);
	    error (0, 0, "\
but CVS uses %s for its own purposes; skipping %s directory", CVSADM, dir);
	    printed_cvs_msg = 1;
	}
	return 0;
    }
    ...
    update_dir = xmalloc (strlen (saved_update_dir) + strlen (dir) + 5);
...
unroll_files_proc (Node * p, void *closure)
{
	...
	update_dir = xmalloc (strlen (save_update_dir) + strlen (p->key) + 5);
