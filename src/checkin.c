	 ...
	 char *options, char *message)
{
    ...
    switch (RCS_checkin (finfo->rcs,
			 finfo->file, message, rev, RCS_FLAGS_KEEPFILE))
    {
	case 0:			/* everything normal */
	    ...
	    if ((!preserve_perms
		 ...
		 && (strcmp (options, "-ko") == 0
		     || strcmp (options, "-kb") == 0))
		|| RCS_cmp_file (finfo->rcs, rev, (char **) NULL,
				 (char *) NULL, options, finfo->file) == 0)
	    {
		/*
	    ...
	    Register (finfo->entries, finfo->file, vers->vn_rcs,
		      vers->ts_user, vers->options, vers->tag, vers->date,
		      (char *) 0);
	    history_write (type, NULL, vers->vn_rcs, finfo->file,
			   finfo->repository);
